...
| `<sandbox-dir>/outputs/` | Stores generated images |
...
SANDBOX_DIR="/tmp/skill-sessions/image-gen-$(date +%s)"
...
Generate a structured JSON file in `<sandbox-dir>/workspace/` with naming pattern: `{descriptive-name}.json`
...
python ./scripts/generate.py \
  --prompt-file <sandbox-dir>/workspace/prompt-file.json \
  --reference-images /path/to/ref1.jpg /path/to/ref2.png \
  --output-file <sandbox-dir>/outputs/generated-image.jpg
  --aspect-ratio 16:9
```
...
Create prompt file: `<sandbox-dir>/workspace/asian-woman.json`
```json
{
  "characters": [{
    "gender": "female",
...
```bash
python ./scripts/generate.py \
  --prompt-file <sandbox-dir>/workspace/cyberpunk-hacker.json \
  --output-file <sandbox-dir>/outputs/cyberpunk-hacker-01.jpg \
  --aspect-ratio 2:3
```
...
```bash
python ./scripts/generate.py \
  --prompt-file <sandbox-dir>/workspace/star-wars-scene.json \
  --reference-images /path/to/character-ref.jpg /path/to/vehicle-ref.jpg \
  --output-file <sandbox-dir>/outputs/star-wars-scene-01.jpg \
  --aspect-ratio 16:9
```
...
- Images are saved in `<sandbox-dir>/outputs/`
